      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {proposal.review_payload_json && (() => {
            const payload = proposal.review_payload_json as Record<string, unknown>;
            const matchedAxes = Array.isArray(payload.matched_axes) ? payload.matched_axes as string[] : [];
            const missingAxes = Array.isArray(payload.missing_axes) ? payload.missing_axes as string[] : [];
            const confidenceFactors = Array.isArray(payload.confidence_factors) ? payload.confidence_factors as string[] : [];
            return (
              ...
              {confidenceFactors.length > 0 && (
                <div className="p-2 bg-white/5 rounded-lg">
                  ...
                  <ul className="text-xs text-slate-300 space-y-0.5">
                    {confidenceFactors.map((factor: string, idx: number) => (
